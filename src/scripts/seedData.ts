...
export async function runAllSeeds() {
  ...
  try {
    ...
    try {
      ...
      await seedManager.logSeedResult('Users', 0, false, error instanceof Error ? error.message : String(error))
    }
    ...
    try {
      ...
      await seedManager.logSeedResult('SubmissionPlans', Array.isArray(plans) ? plans.length : 0, true)
    } catch (error) {
      await seedManager.logSeedResult('SubmissionPlans', 0, false, error instanceof Error ? error.message : String(error))
    }
    ...
    try {
      ...
      await seedManager.logSeedResult('Sections', Array.isArray(sections) ? sections.length : 0, true)
    } catch (error) {
      await seedManager.logSeedResult('Sections', 0, false, error instanceof Error ? error.message : String(error))
    }
    ...
    try {
      ...
      await seedManager.logSeedResult('Questions', Array.isArray(questions) ? questions.length : 0, true)
    } catch (error) {
      await seedManager.logSeedResult('Questions', 0, false, error instanceof Error ? error.message : String(error))
    }
